...
_qr_bytes_cache = None


def _get_qr_bytes():
    """
    Charge l'image QR locale; si absente ou erreur, renvoie le fallback embarqué.
    """
    global _qr_bytes_cache
    if _qr_bytes_cache is not None:
        return _qr_bytes_cache
    try:
        with open(QR_IMAGE_PATH, "rb") as qr_file:
            _qr_bytes_cache = qr_file.read()
        current_app.logger.info("QR code chargé depuis le fichier: %s (len=%d)", QR_IMAGE_PATH, len(_qr_bytes_cache))
    except Exception:
        ...
        try:
            _qr_bytes_cache = base64.b64decode(WEBSITE_QR_DATA_URI_FALLBACK.split(",", 1)[1])
            current_app.logger.info("QR code chargé depuis le fallback embarqué (len=%d)", len(_qr_bytes_cache))
        except Exception:
            _qr_bytes_cache = b""
            current_app.logger.warning("QR code fallback invalide, aucune image attachée.")
    return _qr_bytes_cache
...
def send_email(to, subject, text_body, html_body=None, reply_to=None, cc=None, bcc=None):
    ...
    footer_text = (
        ...
        "QR code : scannez l'image ci-dessous pour ouvrir le site.\n"
    )
    text_with_footer = (text_body or "").rstrip() + footer_text

    html_with_footer = None
    qr_cid = make_msgid(domain="sahera-webapp.ca")
    qr_cid_ref = qr_cid.strip("<>")
    if html_body:
        footer_html = f"""
        <div style="margin-top:24px;padding-top:16px;border-top:1px solid #e5e7eb;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;font-size:14px;line-height:1.6;color:#111827;">
          ...
          <div style="display:flex;align-items:center;gap:12px;align-items:flex-start;">
            <img src="cid:{qr_cid_ref}" alt="QR code vers Sahera" width="120" height="120" style="border:1px solid #e5e7eb;border-radius:8px;" />
...
{footer_html}"""
    ...
    html_part = None
    if html_with_footer:
        html_part = msg.add_alternative(html_with_footer, subtype="html")
    if html_part:
        qr_bytes = _get_qr_bytes()
        if qr_bytes:
            try:
                html_part.add_related(
                    ...
                    maintype="image",
                    subtype="png",
                    cid=qr_cid_ref,  # Content-ID sans chevrons, ils seront ajoutés
                    disposition="inline",
                    filename="sahera-webapp.png",
                )
                current_app.logger.info("QR code attaché en inline (CID=%s, len=%d)", qr_cid_ref, len(qr_bytes))
            except Exception:
                current_app.logger.exception("Impossible d'attacher le QR code dans l'email")
        else:
            current_app.logger.warning("QR code introuvable ou vide, non attaché.")
